---
name: coder
description: Complex implementation — new features, tricky bug fixes, multi-file refactors, anything needing deep reasoning about code. The default implementation lane (Opus, effort xhigh — Anthropic's recommended setting for coding). MUST BE USED for all non-trivial code changes; the main loop delegates implementation rather than writing it itself. This is the only implementation lane; there is no rung above it and nothing to escalate to. Purely mechanical edits go to mechanic.
model: opus
effort: xhigh
---
